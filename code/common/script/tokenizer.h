...
protected:
	...
	bool parsing(char c) override
	{
		...
		if (c != ' ' && c != '_' && c != '-'
			...
			throw std::invalid_argument{ std::format("invalid tag string: `{0}`", std::string_view{ buffer_ + c }) };
	...
	void initialize()
	{
		...
		try
		{
			for (const auto& c : optimize_plain_)
			{
				...
				throw std::invalid_argument{ std::format("invalid token found `{0}`: (int){1}", c, (int)c) };
			}
		}
